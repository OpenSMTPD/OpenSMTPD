...
/* XXX Really */
#ifndef EAI_BADHINTS
#define EAI_BADHINTS 16
#endif
#ifndef EAI_PROTOCOL
#define EAI_PROTOCOL 17
#endif
...
asr_run_dns(struct asr_query *aq, struct asr_result *ar)
{
	...
	case ASR_STATE_PACKET:
		memmove(&ar->ar_sa.sa, AQ_NS_SA(aq), SA_LEN(AQ_NS_SA(aq)));
...
asr_run_cname(struct asr_query *aq, struct asr_result *ar)
{
	...
	case ASR_STATE_READ_FILE:
		...
		if (sockaddr_from_str(&ar->ar_sa.sa, aq->aq_sa.sa.sa_family, tok[0]) == -1)
			break;
		if (SA_LEN(&ar->ar_sa.sa) != SA_LEN(&aq->aq_sa.sa) ||
		    memcmp(&ar->ar_sa.sa, &aq->aq_sa.sa, SA_LEN(&aq->aq_sa.sa)))
			break;
