...
ssl_load_file(const char *name, off_t *len, mode_t perm)
{
	...
	if (fstat(fd, &st) != 0)
		goto fail;
	if (st.st_uid != 0) {
		log_info("info: %s: not owned by uid 0", name);
		errno = EACCES;
		goto fail;
	}
	if (st.st_mode & (S_IRWXU | S_IRWXG | S_IRWXO) & ~perm) {
		strmode(perm, mode);
		log_info("info: %s: insecure permissions: must be at most %s",
		    name, &mode[1]);
		errno = EACCES;
		goto fail;
	}
...
ssl_load_certfile(const char *name, uint8_t flags)
{
	...
	if (strlcpy(key.ssl_name, name, sizeof(key.ssl_name))
	    >= sizeof(key.ssl_name)) {
		log_warn("warn: ssl_load_certfile: certificate name truncated");
		return -1;
	}
	...
	if (s->ssl_ca == NULL) {
		if (errno == EACCES)
			goto err;
		log_info("info: no CA found in %s", certfile);
	}
	...
	if (s->ssl_dhparams == NULL) {
		if (errno == EACCES)
			goto err;
		log_info("info: no DH parameters found in %s", certfile);
		log_info("info: using built-in DH parameters");
	}
