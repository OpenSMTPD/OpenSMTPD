...
queue_imsg(struct mproc *p, struct imsg *imsg)
{
	...
	if (p->proc == PROC_LKA) {
		...
		case IMSG_LKA_ENVELOPE_SUBMIT:
			...
			m_create(p_sessions, IMSG_QUEUE_ENVELOPE_SUBMIT, 0, 0, -1);
			m_add_id(p_sessions, reqid);
			if (ret == 0)
				m_add_int(p_sessions, 0);
			else {
				m_add_int(p_sessions, 1);
				m_add_evpid(p_sessions, evp.id);
			}
			m_close(p_sessions);
			if (ret) {
		...
		case IMSG_LKA_ENVELOPE_COMMIT:
			...
			m_get_id(&m, &reqid);
			m_end(&m);
			m_create(p_sessions, IMSG_QUEUE_ENVELOPE_COMMIT, 0, 0, -1);
			m_add_id(p_sessions, reqid);
			m_add_int(p_sessions, 1);
			m_close(p_sessions);
			return;
	...
	if (p->proc == PROC_SCHEDULER) {
		...
		case IMSG_SCHED_ENVELOPE_DELIVER:
			...
			m_create(p_sessions, IMSG_QUEUE_DELIVER, 0, 0, -1);
			m_add_envelope(p_sessions, &evp);
			m_close(p_sessions);
			return;
		...
		case IMSG_SCHED_ENVELOPE_TRANSFER:
			...
			evp.lasttry = time(NULL);
			m_create(p_sessions, IMSG_QUEUE_TRANSFER, 0, 0, -1);
			m_add_envelope(p_sessions, &evp);
			m_close(p_sessions);
			return;
